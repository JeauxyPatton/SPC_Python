...
# and this script will generate the official TORNADO reports from
...
fname = "{}1950-2019_torn_UTC.csv".format(filepath)
newfname = "{}{}/{}_torn_UTC.csv".format(filepath,outbreak_day,outbreak_day)
print(f'\tThew new CSV file is being written to {newfname}.')
		...
		if line_count == 0:
			...
			newfile_towrite.writerow([row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],
				...
				if ((storm_datetime >= begin_datetime) and (storm_datetime <= end_datetime)):
					print(f'\tThe tornado report time is {storm_datetime}.')
				...
				#print(f'\trow[0] represents {row[0]}.')
					newfile_towrite.writerow([row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],
						...
						row[20],row[21],row[22],row[23],row[24],row[25],row[26],row[27],row[28]])
					line_count += 1
	print(f'Processed {line_count} tornado reports.')
